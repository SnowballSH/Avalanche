...
pub const Piece = enum(u8) {
    ...
    pub inline fn color(self: Piece) Color {
        return @enumFromInt((@intFromEnum(self) & 0b1000) >> 3);
    }
...
pub const Square = enum(u8) {
    ...
    pub inline fn diagonal(self: Square) i32 {
        return @intCast(7 + @intFromEnum(self.rank()) - @intFromEnum(self.file()));
    }

    pub inline fn anti_diagonal(self: Square) i32 {
        return @intCast(@intFromEnum(self.rank()) + @intFromEnum(self.file()));
    }
...
pub const Move = packed struct {
    ...
    pub inline fn new_from_to(from: Square, to: Square) Move {
        return Move{ .flags = 0, .from = @intCast(@intFromEnum(from)), .to = @intCast(@intFromEnum(to)) };
    }

    pub inline fn new_from_to_flag(from: Square, to: Square, flag: MoveFlags) Move {
        return Move{ .flags = @intFromEnum(flag), .from = @intCast(@intFromEnum(from)), .to = @intCast(@intFromEnum(to)) };
    }
    ...
    pub fn new_from_string(pos: *position.Position, move: []const u8) Move {
        ...
        defer list.deinit();
        const f: u6 = @intCast(@intFromEnum(Square.new(@enumFromInt(move[0] - 'a'), @enumFromInt(move[1] - '1'))));
        const t: u6 = @intCast(@intFromEnum(Square.new(@enumFromInt(move[2] - 'a'), @enumFromInt(move[3] - '1'))));
