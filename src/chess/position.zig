...
pub const Position = struct {
    ...
    pub fn set_fen(self: *Position, fen: []const u8) void {
        ...
        for (bd) |ch| {
            if (std.ascii.isDigit(ch)) {
                ...
                self.add_piece(@enumFromInt(utils.first_index(u8, types.PieceString[0..], ch).?), @enumFromInt(sq));
                sq += 1;
            }
        }
        ...
        if (ep.len == 2) {
            for (types.SquareToString, 0..) |sq_str, i| {
                if (std.mem.eql(u8, ep, sq_str)) {
                    self.history[self.game_ply].ep_sq = @enumFromInt(i);
    ...
    pub inline fn in_check(self: Position, comptime color: types.Color) bool {
        const king: types.Piece = comptime types.Piece.new_comptime(color, types.PieceType.King);
        const opp = comptime if (color == types.Color.White) types.Color.Black else types.Color.White;
        return self.attackers_from(opp, @enumFromInt(types.lsb(self.piece_bitboards[king.index()])), self.all_pieces(types.Color.White) | self.all_pieces(types.Color.Black)) != 0;
    }
    ...
    pub fn generate_legal_moves(self: *Position, comptime color: types.Color, list: *std.ArrayList(types.Move)) void {
        ...
        const our_king: types.Square = @enumFromInt(types.lsb(self.piece_bitboards[types.Piece.new_comptime(color, types.PieceType.King).index()]));
        const their_king: types.Square = @enumFromInt(types.lsb(self.piece_bitboards[types.Piece.new_comptime(opp, types.PieceType.King).index()]));
        ...
        const rel_north = comptime if (color == types.Color.White) types.Direction.North else types.Direction.South;
        const rel_south = comptime if (color == types.Color.White) types.Direction.South else types.Direction.North;
        const rel_northwest = comptime if (color == types.Color.White) types.Direction.NorthWest else types.Direction.SouthEast;
        const rel_northeast = comptime if (color == types.Color.White) types.Direction.NorthEast else types.Direction.SouthWest;
        ...
        switch (types.popcount(self.checkers)) {
            ...
            1 => {
                    ...
                    types.Piece.new_comptime(opp, types.PieceType.Knight) => {
                        ...
                        while (b1 != 0) {
                            ...
                            if ((&self.mailbox)[psq.index()].piece_type() == types.PieceType.Pawn and (types.SquareIndexBB[psq.index()] & types.MaskRank[types.Rank.RANK7.relative_rank(color).index()]) != 0) {
                                ...
                                list.append(types.Move.new_from_to_flag(psq, checker_sq, @enumFromInt(types.PC_BISHOP))) catch {};
                            } else {
                ...
                if (ep != types.Square.NO_SQUARE) {
                    ...
                    if (b1 != 0) {
                        list.append(types.Move.new_from_to_flag(@enumFromInt(types.lsb(b1)), ep, types.MoveFlags.EN_PASSANT)) catch {};
                    }
                }
        ...
        if (b1 != 0) {
            ...
            while (b2 != 0) {
                ...
                list.append(types.Move.new_from_to_flag(sq.sub(rel_north), sq, @enumFromInt(types.PR_QUEEN))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_north), sq, @enumFromInt(types.PR_ROOK))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_north), sq, @enumFromInt(types.PR_KNIGHT))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_north), sq, @enumFromInt(types.PR_BISHOP))) catch {};
            }
            ...
            while (b2 != 0) {
                ...
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northwest), sq, @enumFromInt(types.PC_QUEEN))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northwest), sq, @enumFromInt(types.PC_ROOK))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northwest), sq, @enumFromInt(types.PC_KNIGHT))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northwest), sq, @enumFromInt(types.PC_BISHOP))) catch {};
            }

            while (b3 != 0) {
                sq = types.pop_lsb(&b3);

                list.append(types.Move.new_from_to_flag(sq.sub(rel_northeast), sq, @enumFromInt(types.PC_QUEEN))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northeast), sq, @enumFromInt(types.PC_ROOK))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northeast), sq, @enumFromInt(types.PC_KNIGHT))) catch {};
                list.append(types.Move.new_from_to_flag(sq.sub(rel_northeast), sq, @enumFromInt(types.PC_BISHOP))) catch {};
            }
        }
