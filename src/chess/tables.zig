...
pub fn init_rook_attacks() void {
    ...
    while (sq <= @intFromEnum(types.Square.h8)) : (sq += 1) {
        ...
        index = index >> @intCast(RookAttackShifts[sq]);
        RookAttacks[sq][index] = get_rook_attacks_for_init(@enumFromInt(sq), subset);
...
pub inline fn get_rook_attacks(square: types.Square, occ: types.Bitboard) types.Bitboard {
    return RookAttacks[square.index()][((occ & RookAttackMasks[square.index()]) *% RookMagics[square.index()]) >> @intCast(RookAttackShifts[square.index()])];
}
...
inline fn get_bishop_attacks_for_init(square: types.Square, occ: types.Bitboard) types.Bitboard {
    return sliding_attack(square, occ, types.MaskDiagonal[@intCast(square.diagonal())]) | sliding_attack(square, occ, types.MaskAntiDiagonal[@intCast(square.anti_diagonal())]);
}
...
pub fn init_bishop_attacks() void {
    ...
    while (sq <= @intFromEnum(types.Square.h8)) : (sq += 1) {
        ...
        index = index >> @intCast(BishopAttackShifts[sq]);
        BishopAttacks[sq][index] = get_bishop_attacks_for_init(@enumFromInt(sq), subset);
...
pub inline fn get_bishop_attacks(square: types.Square, occ: types.Bitboard) types.Bitboard {
    return BishopAttacks[square.index()][((occ & BishopAttackMasks[square.index()]) *% BishopMagics[square.index()]) >> @intCast(BishopAttackShifts[square.index()])];
}
    ...
    while (sq1 <= @intFromEnum(types.Square.h8)) : (sq1 += 1) {
        ...
        while (sq2 <= @intFromEnum(types.Square.h8)) : (sq2 += 1) {
            ...
            if (types.file_plain(sq1) == types.file_plain(sq2) or types.rank_plain(sq1) == types.rank_plain(sq2)) {
                SquaresBetween[sq1][sq2] = get_rook_attacks_for_init(@enumFromInt(sq1), sqs) & get_rook_attacks_for_init(@enumFromInt(sq2), sqs);
            } else if (types.diagonal_plain(sq1) == types.diagonal_plain(sq2) or types.anti_diagonal_plain(sq1) == types.anti_diagonal_plain(sq2)) {
                SquaresBetween[sq1][sq2] = get_bishop_attacks_for_init(@enumFromInt(sq1), sqs) & get_bishop_attacks_for_init(@enumFromInt(sq2), sqs);
            } else {
...
pub fn init_line_between() void {
    ...
    while (sq1 <= @intFromEnum(types.Square.h8)) : (sq1 += 1) {
        ...
        while (sq2 <= @intFromEnum(types.Square.h8)) : (sq2 += 1) {
            if (types.file_plain(sq1) == types.file_plain(sq2) or types.rank_plain(sq1) == types.rank_plain(sq2)) {
                LineOf[sq1][sq2] = get_rook_attacks_for_init(@enumFromInt(sq1), 0) & get_rook_attacks_for_init(@enumFromInt(sq2), 0) | types.SquareIndexBB[sq1] | types.SquareIndexBB[sq2];
            } else if (types.diagonal_plain(sq1) == types.diagonal_plain(sq2) or types.anti_diagonal_plain(sq1) == types.anti_diagonal_plain(sq2)) {
                LineOf[sq1][sq2] = get_bishop_attacks_for_init(@enumFromInt(sq1), 0) & get_bishop_attacks_for_init(@enumFromInt(sq2), 0) | types.SquareIndexBB[sq1] | types.SquareIndexBB[sq2];
            } else {
                LineOf[sq1][sq2] = 0;
            }
        }
...
pub fn init_pseudo_legal() void {
    ...
    while (sq <= @intFromEnum(types.Square.h8)) : (sq += 1) {
        PseudoLegalAttacks[@intFromEnum(types.PieceType.Bishop)][sq] = get_bishop_attacks_for_init(@enumFromInt(sq), 0);
        PseudoLegalAttacks[@intFromEnum(types.PieceType.Rook)][sq] = get_rook_attacks_for_init(@enumFromInt(sq), 0);
