...
pub inline fn distance_eval(pos: *position.Position, comptime white_winning: bool) i32 {
    const k1: types.Square = @enumFromInt(types.lsb(pos.piece_bitboards[types.Piece.WHITE_KING.index()]));
    const k2: types.Square = @enumFromInt(types.lsb(pos.piece_bitboards[types.Piece.BLACK_KING.index()]));

    const r1: i32 = @intCast(k1.rank().index());
    const r2: i32 = @intCast(k2.rank().index());
    const c1: i32 = @intCast(k1.file().index());
    const c2: i32 = @intCast(k2.file().index());
...
pub fn evaluate_comptime(pos: *position.Position, comptime color: types.Color) i32 {
    ...
    result = @divTrunc(result * (700 + @divTrunc(pos.phase_material(), 32) - @as(i32, pos.history[pos.game_ply].fifty) * 5), 1024);
