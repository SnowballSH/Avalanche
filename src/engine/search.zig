...
pub fn init_lmr() void {
    ...
    while (depth < 64) : (depth += 1) {
        ...
        while (moves < 64) : (moves += 1) {
            const a = parameters.LMRWeight * std.math.log(f32, std.math.e, @floatFromInt(depth)) * std.math.log(f32, std.math.e, @as(f32, @floatFromInt(moves))) + parameters.LMRBias;
            QuietLMR[depth][moves] = @intFromFloat(@floor(a));
        }
    }
}
...
pub const Searcher = struct {
    ...
    pub inline fn should_not_continue(self: *Searcher, factor: f32) bool {
        return self.stop or (self.thread_id == 0 and self.iterative_deepening_depth > self.min_depth and ((self.soft_max_nodes != null and self.nodes >= self.soft_max_nodes.?) or (!self.force_thinking and self.timer.read() / std.time.ns_per_ms >= @min(self.max_millis, @as(u64, @intFromFloat(@floor(@as(f64, @floatFromInt(self.ideal_time)) * factor)))))));
    }
    ...
    pub fn is_draw(self: *Searcher, pos: *position.Position, threefold: bool) bool {
        ...
        if (self.hash_history.items.len > 1) {
            ...
            const limit: i16 = @intCast(index - @as(i32, pos.history[pos.game_ply].fifty) - 1);
            var count: u8 = 0;
            const threshold: u8 = if (threefold) 2 else 1;
            while (index >= limit and index >= 0) {
                if (self.hash_history.items[@intCast(index)] == pos.hash) {
                    count += 1;
                    if (count >= threshold) {
                        return true;
                    }
    ...
    pub fn negamax(self: *Searcher, pos: *position.Position, comptime color: types.Color, depth_: usize, alpha_: i32, beta_: i32, comptime is_null: bool, comptime node: NodeType, comptime cutnode: bool) i32 {
        ...
        var beta = beta_;
        var depth = depth_;
        const opp_color = comptime if (color == types.Color.White) types.Color.Black else types.Color.White;
        ...
        if (!in_check and !on_pv and self.exclude_move[self.ply].to_u16() == 0) {
            ...
            if (!is_null and depth >= 3 and self.ply >= self.nmp_min_ply and nmp_static_eval >= beta and has_non_pawns) {
                ...
                r += @intCast(@min(4, @divTrunc((static_eval - beta), parameters.NMPBetaDivisor)));
                r = @min(r, depth);
                ...
                if (null_score >= beta) {
                    ...
                    self.nmp_min_ply = @intCast(self.ply + (depth - r) * 3 / 4);
        ...
        while (index < move_size) : (index += 1) {
            ...
            if (on_pv and legals == 1) {
                ...
                if (!in_check and depth >= 3 and index >= min_lmr_move and (!is_capture or !is_winning_capture)) {
                    ...
                    const rd: usize = @intCast(std.math.clamp(@as(i32, @intCast(new_depth)) - reduction, 1, @as(i32, @intCast(new_depth + 1))));
