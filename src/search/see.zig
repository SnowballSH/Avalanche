...
const PT_VAL: [8]i16 = .{ 95, 290, 290, 290, 510, 510, 920, 10000 };
...
fn evaluate_internal(attacking_piece: u8, target_piece: u8, attackers: u8, defenders: u8) i16 {
    ...
    var new_attackers = attackers & ~(@as(u8, 1) << @intCast(u3, attacking_piece));
    var new_attacking_piece = if (defenders == 0) 0 else @intCast(u8, @ctz(u64, get_lsb(defenders)));
...
pub fn get_see(attacking_piece: u8, target_piece: u8, attackers: u8, defenders: u8) i16 {
    ...
    var new_attackers = attackers & ~(@as(u8, 1) << attacking_idx);
